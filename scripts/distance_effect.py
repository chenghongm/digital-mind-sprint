    ...
    for n, rec in enumerate(recs, 1):
        ...
        for d in args.distances:
            ...
            for i in range(d):
                ...
                msgs.append({"role": "assistant", "content": ft[i]["model_text"]})
            t1 = time.time()
            ...
            low += 0 if ok else 1
            row["readings"][str(d)] = {
                "p_a": p_a, "mass": mass, "p_orders": orders, "valid": ok,
                "ctx_tokens": len(rr.tok.apply_chat_template(
                    msgs, add_generation_prompt=True)),
                "secs": round(time.time() - t1, 1)}
        out.append(row)
        vals = " ".join(f"{d}:{row['readings'][str(d)]['p_a']:.2f}"
                        for d in args.distances)
        print(f"[{n}/{len(recs)}] {rec['conv_id']:<28} {vals}", flush=True)

    json.dump({"run": args.run, "fill": args.fill,
               "distances": args.distances, "min_mass": args.min_mass,
               "low_mass": low, "conversations": out},
              open(args.out, "w"), ensure_ascii=False, indent=1)
    secs = time.time() - t0
    print(f"\n[json] {args.out}")
    print(f"[mass] {low} of {len(recs) * len(args.distances)} readings below "
          f"{args.min_mass}")
